...
def main(config):
    ...
    manual_sync_realsense = config['manual_sync_realsense']
    manual_sync_vicon_frame = config['manual_sync_vicon_frame']
    output_csv_path = config['output_csv_path']
    realsense_dir = config['realsense_dir']
    ...
    image_files = os.listdir(realsense_dir)
    image_data = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Synchronize RealSense frames with Vicon data.')
    parser.add_argument('config', help='Path to the configuration file (config.yaml)')
